...
const Game = () => {
  ...
  const GameModeButton = ({ title, icon, description, onClick }) => (
    <div 
      className="game-mode-button bg-white hover:bg-indigo-50 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 hover:-translate-y-1 cursor-pointer overflow-hidden h-full"
      onClick={onClick}
    >
  ...
  return (
    <div className="bg-gradient-to-b from-indigo-50 to-purple-50 min-h-screen py-8">
      <div className="container mx-auto px-4 flex flex-col items-center">
        <h1 className="text-center text-4xl font-bold text-indigo-800 mb-8">Deepfake Detection Game</h1>
        ...
        <div className="weekly-leaderboard bg-white rounded-xl shadow-md p-6 mb-8 w-full max-w-3xl">
          <div className="leaderboard-header flex justify-between items-center mb-4">
            <h2 className="text-2xl font-bold text-indigo-800">Weekly Leaderboard</h2>
          ...
          {loading ? (
            ...
            <div className="leaderboard-list max-h-60 overflow-y-auto pr-2">
              {leaderboardData.map((player, index) => (
                <div key={player.id} className="leaderboard-item flex items-center p-3 border-b border-indigo-100 hover:bg-indigo-50 transition-colors">
                  ...
                  <div className="player-info flex-grow">
                    <div className="username font-medium text-indigo-900">{player.username}</div>
                    <div className="additional-info text-xs text-indigo-600">Level {player.level}</div>
                  </div>
                  <div className="score font-bold text-lg text-indigo-800">{player.score}</div>
                </div>
              ))}
            </div>
          )}
