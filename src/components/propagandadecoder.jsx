...
const PropagandaDecoder = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-indigo-50 to-purple-100 flex flex-col items-center justify-center px-6 py-12">
      {/* ✅ Header */}
      <motion.h1
      ...
      <p className="text-indigo-700 text-lg mb-8 text-center max-w-2xl">
        Rewrite the following biased text into <strong className="text-purple-600">neutral language</strong> to score points!
      </p>
      ...
      {/* ✅ Input Box */}
      <textarea
        className="mt-6 p-4 w-full max-w-3xl rounded-xl border border-indigo-300 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-shadow shadow-sm"
        rows="4"
      ...
      <button
        onClick={analyzeText}
        className="mt-6 px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold rounded-full shadow-lg hover:from-indigo-700 hover:to-purple-700 transition-transform transform hover:scale-105"
      ...
      {score !== null && (
        <motion.div
          className="mt-8 p-6 bg-white shadow-lg rounded-xl max-w-lg border border-indigo-200 text-center"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
        >
          <h3 className="text-2xl font-bold text-indigo-800">Your Score: {score}/100</h3>
          <p className={`mt-2 text-lg font-medium ${score >= 80 ? "text-green-600" : "text-purple-600"}`}>
            {feedback}
          </p>
        </motion.div>
      )}
      ...
      <div className="mt-6 text-indigo-700 font-medium text-lg">
        🔥 Current Streak: <span className="text-purple-800 font-bold">{streak}</span>
      </div>
    </div>
  );
};
